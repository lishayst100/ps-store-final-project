...
import { url } from "../../features/utils";
...
const ListGames = () => {
  ...
  const filterGames = (endpoint) => {
    fetch(`http://localhost:3001/api/games/${endpoint}games`)
      .then((res) => res.json())
  ...
  const searchGames = async(e) => {
    ...
    if(key){
      let result = await fetch(`http://localhost:3001/api/games/search/${key}`);
      ...
      <div className="d-flex gap-3 justify-content-center flex-column shadow-lg p-4 rounded">
        ...
        <div className={showFilters ? "wrapper open" : "wrapper"}>
          <div className="expandable d-flex flex-column gap-4">
            ...
            <div className="filter-btns d-flex gap-2 justify-content-center">
              <button
